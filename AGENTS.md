...
- Do not change code, tests, scripts, workflows, configuration, or other executable behavior without an existing GitHub issue.
- When work is identified, create the issue before editing code.
- Create issues in `jcszymansk/claude-review-loop`, this fork, not the upstream repository.
- Include the issue number or URL in the change context before implementation starts.
- This policy applies to every agent and contributor working in this repository.

Documentation-only changes and updates to this policy do not require a separate issue unless they also change executable behavior.
...
**Setup lives in two places.** `commands/review-loop.md` embeds its own setup logic in its first ```` ```bash ```` fence: it calls `resolve-pr-url.sh`, `resolve-reviewer.sh`, `resolve-max-rounds.sh`, `resolve-review-timeout.sh`, `capture-worktree-tree.sh` and `ensure-codex-config.sh`, then writes the state JSON. `scripts/setup-review-loop.sh` is a parallel CLI implementation of the same logic, used by tests. Changes to setup must be made in both. `tests/pr-scope.sh` extracts and executes that first bash fence from the command file with `awk`, so restructuring the command doc can break it.

**`hooks/stop-hook.sh`** has two phases:

- `task`: computes `task-diff.md` (against the baseline tree captured at setup) and the scope diff (PR diff via `curl`, else branch diff), renders the prompt, writes a runner script `.claude/review-loop-run-<reviewer>.sh` from a heredoc, runs it synchronously, transitions to `addressing`, and blocks with `prompts/addressing-review.md`. The runner's watchdog enforces `review_timeout` (default 1800s). The `timeout` in `hooks/hooks.json` (14400s) is only a backstop, because Claude Code discards the output of a hook it cancels; the hook reads that number with `scripts/read-hook-timeout.sh` and lowers the review limit so the review ends 60s before it, counting from the start of the Stop event.
...
**Reviewer dispatch**: the generated runner records its PID and calls `scripts/run-reviewer.sh <reviewer> <prompt-file> <review-file>`, which builds the per-provider command. It adopts captured stdout as the review only when the reviewer did not write a valid artifact itself. A watchdog in the runner stops the dispatcher process group and tree when `review_timeout` passes, and the runner then records the timeout in `.claude/review-loop-timed-out`, which the hook, not exit status 124, uses to tell a timeout from a failed reviewer. `/cancel-review` runs `scripts/cancel-review-loop.sh`, which kills the recorded process tree and removes runtime files.
...
- Adding a reviewer touches the reviewer `case` blocks in `stop-hook.sh`, `commands/review-loop.md`, `setup-review-loop.sh`, `resolve-reviewer.sh`, `run-reviewer.sh`, and the generated-file lists duplicated in `cleanup_generated_files` (hook) and `cancel-review-loop.sh`. Any new runtime file under `.claude/` also goes into both lists.
